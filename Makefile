# This Makefile is for the SQCAS extension to perl.
#
...
#     ABSTRACT_FROM => q[SQCAS.pm]
#     AUTHOR => q[Sean Quinlan <seanq@darwin.bu.edu>]
#     NAME => q[SQCAS]
#     PREREQ_PM => {  }
#     VERSION_FROM => q[SQCAS.pm]
...
manifypods : pure_all SQCAS.pm
	@$(POD2MAN) \
	SQCAS.pm \
	$(INST_MAN3DIR)/SQCAS.$(MAN3EXT)
...
subdirs ::
	@cd Admin && $(MAKE) -f $(FIRST_MAKEFILE) all $(PASTHRU)

subdirs ::
	@cd Auth && $(MAKE) -f $(FIRST_MAKEFILE) all $(PASTHRU)

subdirs ::
	@cd DB && $(MAKE) -f $(FIRST_MAKEFILE) all $(PASTHRU)

subdirs ::
	@cd _config && $(MAKE) -f $(FIRST_MAKEFILE) all $(PASTHRU)
...
clean ::
	-cd Admin && $(TEST_F) Makefile && $(MAKE) clean
	-cd Auth && $(TEST_F) Makefile && $(MAKE) clean
	-cd DB && $(TEST_F) Makefile && $(MAKE) clean
	-cd _config && $(TEST_F) Makefile && $(MAKE) clean
...
realclean purge ::  clean
	-cd Admin && $(TEST_F) Makefile.old && $(MAKE) -f Makefile.old realclean
	-cd Admin && $(TEST_F) Makefile && $(MAKE)  realclean
	-cd Auth && $(TEST_F) Makefile.old && $(MAKE) -f Makefile.old realclean
	-cd Auth && $(TEST_F) Makefile && $(MAKE)  realclean
	-cd DB && $(TEST_F) Makefile.old && $(MAKE) -f Makefile.old realclean
	-cd DB && $(TEST_F) Makefile && $(MAKE)  realclean
	-cd _config && $(TEST_F) Makefile.old && $(MAKE) -f Makefile.old realclean
	-cd _config && $(TEST_F) Makefile && $(MAKE)  realclean
	rm -rf $(INST_AUTODIR) $(INST_ARCHAUTODIR)
	rm -rf $(DISTVNAME)
	rm -f  blib/lib/SQCAS.pm
...
$(MAKE_APERL_FILE) : $(FIRST_MAKEFILE)
	...
	@$(PERLRUNINST) \
		Makefile.PL DIR=Admin:Auth:DB:_config \
...
test :: $(TEST_TYPE)
	@cd Admin && $(TEST_F) Makefile && $(MAKE) test $(PASTHRU)
	@cd Auth && $(TEST_F) Makefile && $(MAKE) test $(PASTHRU)
	@cd DB && $(TEST_F) Makefile && $(MAKE) test $(PASTHRU)
	@cd _config && $(TEST_F) Makefile && $(MAKE) test $(PASTHRU)
...
pm_to_blib: $(TO_INST_PM)
	...
	-e "pm_to_blib({qw{SQCAS.pm blib/lib/SQCAS.pm}},'$(INST_LIB)/auto','$(PM_FILTER)')"
	@$(TOUCH) $@
